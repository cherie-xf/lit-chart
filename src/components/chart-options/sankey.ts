...
const sankeyOption: EChartOption<EChartOption.SeriesSankey> = {
  tooltip: {
    trigger: 'item'
  },
  animation: false,
...
const getOptionByData = (chartData: ChartData<EChartOption.SeriesSankey>): EChartOption => {
...
const getOptionByChartdataset = (
  ...
  if (Array.isArray(source) && source.length) {
    const cols: Array<string> = source.slice(0, 1)[0];
    if (cols.length > 1) {
      const fields: Array<string> = cols.slice(0, -1);
      const valueFrom: string = cols[cols.length - 1];
      const links = fields.reduce((arr: Record<string, string>[], field, idx) => {
        if (fields[idx + 1]) {
      ...
      const nodes: Record<string, []> = fields.reduce((res, nodeName) => {
        return {
          ...res,
          [nodeName]: Array.from(new Set(rows.map(row => row[nodeName]).filter(d => d)))
        };
      }, {});

      let seriesNode = Object.keys(nodes).reduce((acc : Array<any>, field:string) => {
        let subNodes = nodes[field].map(value => {
          ...
          return {
            ...
            field: field,
            oData: oData,
            value: valueFrom ? oData.reduce((res, row) => (res += Number(row[valueFrom])), 0) : undefined
          };
        });
        return [...acc, ...subNodes];
      }, []);
      ...
      const valLinks = links.reduce((acc: Array<any>, link) => {
        ...
        const linkArray = sources.reduce((res: Array<any>, value) => {
          ...
          const crossArr = array.map((obj, index) => [obj, { key: link.target, value: targets[index] }]);
          return [...res, ...crossArr];
        }, []);
      ...
      let seriesLinks = valLinks
        .map(link => {
          ...
          let matchArr = rows.filter(
            d => d[sourceKey] && d[sourceKey] === sourceVal && d[targetKey] && d[targetKey] === targetVal
          );
          return {
            source: sourceVal,
            target: targetVal,
            value: valueFrom ? matchArr.reduce((res, d) => (res += Number(d[valueFrom])), 0) : matchArr.length,
            oData: matchArr
          };
        })
...
export default { option: { ...sankeyOption }, getOptionByData, getOptionByChartdataset, labelFormat };